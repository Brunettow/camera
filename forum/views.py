#functions that defined in urls
...
def login(req):
    ...
    result=run_statement(f"SELECT * FROM Manager WHERE username='{username}' and password='{password}';") #Run the query in DB
...
def homePage(req):
    ...
    return render(req,'homePage.html',{"username":username,"get_count":getCountForm,"action_fail":isFailed})
  
    
    
def getProductCount(req):
    isFailed=req.GET.get("fail",False)
    cameraID = req.POST["cameraID"]
    print(cameraID)

    try:
        url = "http://192.168.0.32/rcp.xml?command=0x0b4a&type=p_octet&direction=READ"
        url2 = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,litecoin&vs_currencies=usd,eur,gbp"
        result = requests.get(url2)
        response = result.text
        print(response)
        return render(req,'getProductCount.html',{"response":response,"action_fail":isFailed})
    except:
